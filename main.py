...
def save_enriched_url_to_bq(enriched_df):
    ...
    job = client.load_table_from_dataframe(enriched_df, table_to_send_to, job_config=job_config)
    job.result()
    print(Fore.GREEN + f"Loaded to BigQuery\n" + Style.RESET_ALL)

def choose_api_key(keys:List,use_timer:bool=False):
    """
    Randomly choose an API key from a """
